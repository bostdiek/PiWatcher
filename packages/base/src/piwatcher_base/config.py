...
class Settings(BaseSettings):
    ...
    frame_storage_path: Path = Field(
        default=Path("/mnt/nvme/piwatcher/frames"),
    ...
    ntfy_topic: str = Field(
        default="piwatcher", validation_alias=AliasChoices("NTFY_TOPIC", "PIWATCHER_NTFY_TOPIC")
    )
    ntfy_url: str = Field(
        default="https://ntfy.sh", validation_alias=AliasChoices("NTFY_URL", "PIWATCHER_NTFY_URL")
    )
    ...
    cooldown_temp_c: float = Field(
        default=60.0, validation_alias=AliasChoices("COOLDOWN_TEMP_C", "PIWATCHER_COOLDOWN_TEMP_C")
    )
