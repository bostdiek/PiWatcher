...
from .transfer import upload_event, wifi_off, wifi_on
...
def run_loop(camera: Camera, config: CameraConfig) -> None:
    ...
    while not _shutdown_requested:
        ...
        if session.should_capture_frame():
            frame_path = _capture_main_frame(camera, config.frame_queue_dir)
            session.on_frame_captured(frame_path)

        state = session.tick()
        if state is CaptureState.TRANSFERRING:
            frames = session.finish()
            _transfer_frames(frames, config, heartbeat, event_start, startup_time)
            event_start = None
...
def _capture_main_frame(camera: Camera, frame_queue_dir: Path) -> Path:
